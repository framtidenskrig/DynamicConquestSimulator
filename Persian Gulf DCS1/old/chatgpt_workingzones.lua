...
local zoneDrawings = {}
local colors = {
    Red = {1, 0, 0, 0.8},   -- Red color with 70% opacity
    Blue = {0, 0, 1, 0.8},  -- Blue color with 70% opacity
    Green = {0, 1, 0, 0.8} -- Green color with 70% opacity
}

-- Function to update zone drawings on the map
function updateZoneDrawing(zoneName)
    local status = zoneStatus[zoneName]
    local color = colors.Red
    local text = "Red Controlled"
    if status.coalition == "Blue" then
        color = colors.Blue
        text = "Blue Controlled"
    elseif status.coalition == "Neutral" then
        color = colors.Green
        text = "Neutral"
    end

    -- Update the text and color based on the current status
    local infoText = string.format("%s - %s - Supplies: %d", zoneName, text, zoneSupplies[zoneName])
    if status.building then
        infoText = infoText .. string.format(" - Building: %s - %d min left", status.building, math.ceil(status.timeRemaining / 60))
    end

    zoneDrawings[zoneName]:DrawZone(zoneStatus.coalition, color)
end

-- Function to handle construction activation and management
...
for _, zoneName in ipairs(zones) do
    MESSAGE:New("Init started for zone: " .. zoneName, 15, "INFO"):ToAll()
    
    -- Find the zone by its name defined in the DCS Mission Editor
    ...
    zoneObjects[zoneName] = zoneObject
    ...
    MESSAGE:New("Init midway for zone: " .. zoneName, 15, "INFO"):ToAll()

    -- Draw initial zone with Red color
    zoneDrawings[zoneName] = ZONE_RADIUS:New(zoneName, zoneObject:GetVec2(), zoneObject:GetRadius())
    if not zoneDrawings[zoneName] then
        MESSAGE:New("Failed to create drawing for zone: " .. zoneName, 15, "ERROR"):ToAll()
        return
    end
    zoneDrawings[zoneName]:DrawZone(1, colors.Red)

    MESSAGE:New("Init ending for zone: " .. zoneName, 15, "INFO"):ToAll()

    -- Start off as Red controlled
    ActivateDefense(zoneName, "Red")
    MESSAGE:New("Init done for zone: " .. zoneName, 15, "INFO"):ToAll()
end
